...
def calculate_eigen_vectors(H, H_prime, sigma):
    ...
    #cos_similarity_Cov = torch.vdot(covariance_matrix,covariance_matrix_prime)
    #max_cos_similarity_Cov = torch.max(cos_similarity_Cov)     #should take max or min is still confusion
    #I need to use dot product of two vectors find the max values can be away from zero or 90 degree
    #batch_cos_sim = batch_cosine_similarity(matrix1, matrix2)
...
def process_dataset(dataset, h_theta_model, sigma):
    ...
    for i in range(num_samples - 1):
        q = torch.tensor(joint_positions[i]).float()             # Ensure tensor is float
        q_prime = torch.tensor(joint_positions[i + 1]).float()   # Ensure tensor is float
        delta_q= q - q_prime
        ...
        # magnitude of the median delta_q
        magnitude_median_delta_q = torch.norm(median_delta_q)
        # delta_desired
        delta_desired = median_delta_q
        #delta_desired=torch.median(torch.stack(delta_q_list), dim=0).values    #median      
        # delta_q_desired
        delta_q_desired = [(delta_q / magnitude) * magnitude_median_delta_q for delta_q, magnitude in zip(delta_q_list, magnitude_delta_q_list)]

        # if magnitude != 0:
        #     normalized_delta_q = delta_q / magnitude
        # else:
        #     normalized_delta_q = delta_q  

        #delta_q_desired= (delta_q / magnitude_delta_q_list) * |delta_desired|         
        H = h_theta_model(q)
...
#     """
#     Computes the cosine similarity between corresponding rows of two matrices.
#     Returns:
#     torch.Tensor: The cosine similarity for each pair of corresponding rows.
#     """
#     dot_products = torch.sum(matrix1 * matrix2, dim=1)
#     norms_matrix1 = torch.norm(matrix1, dim=1)
#     norms_matrix2 = torch.norm(matrix2, dim=1)
#     cosine_sims = dot_products / (norms_matrix1 * norms_matrix2)
#     return cosine_sims
